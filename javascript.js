$(document).ready(function(){
    var i;
    var quiz=[]; // quiz dizisi oluşturduk
    var Count; // sayaç oluşturduk
    var imageChoice; 
    var correct=0; //doğru sayısı için bir değişken oluşturduk
    var missed =0; //yanlış sayısı için bir değişken oluşturduk
    var attempted = 0; // işaretlenen soru sayısı için bir değişken oluşturduk
    var rightAns; 
    var intervalTimer; // saniye için bir değişken oluşturduk
    ...
    $("#new-question").on("click",displayNewQuestion); //new qustiona tıkladığımızda yebi soruyu bize getirir
    $("#new-question").on("click",displayStats); //istatistik kısmını getirir

function temizle() {
    document.getElementById("deneme").style.visibility = "hidden"; // id si deneme olan kısmı gizler 
}
    ...
    function Attempted(){
        ...
        if (userChoice == quiz[i].ans){
            ...
            correct += 1; // doğru cevabı 1 arttırır
        }
        else {
            $("#message").html('Yanlış Cevap verdin !');
            missed += 1; // yanlış cevabı 1 arttırır
        }
    ...
    function quizBuild(){
        quiz[1] = new quizConstructor('Trafikte hangi araçların arkasında "Dolu" ya da "Boş" yazar','Akaryakıt tankeri','Öğrenci servisi','Harfiyat kamyonu','Yolcu otobüsü',1,"",false);
        quiz[2] = new quizConstructor('"Uyuyamadım" anlamındaki söz hangisidir','Uyku atmadı','Uyku kalmadı','Uyku tutmadı','Uyku yatmadı',3,"",false,);
        quiz[3] = new quizConstructor('Yanda yer alan görüntüdeki mimari yapı hangisidir ','Şifaiye Medresesi','Gök Medrese','Buruciye Medresesi','Sivas Ulu Cami',2,"./img/q0.jpg",false);
        quiz[4] = new quizConstructor('Videoda bağlama ile çalınan eser hangisidir ','Güzelliğin On Para Etmez','Gesi Bağları','Çırpınıp İçinde Döndüğüm  Deniz','Kul Olayım Kalem Tutan Ellere',4,"",false,"./img/q1.mp4");
    ...
    function displayStats(){
        $(".stats").html("<h4> Doğru Sayısı: "+correct+'<br>'+"Yanlış Sayısı: " + missed + '<br>' +"İşaretlenen Soru Sayısı: " +attempted+ '</h4>');
    }
